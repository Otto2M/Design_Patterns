...
public class UredZaDostavu {
  ...
  public HashMap<Paket, Date> mapa = new HashMap<>();

  public UredZaDostavu(UredZaPrijem uzp, List<Vozilo> vozila) {
    this.vozila = vozila;
    this.uzp = uzp;
  }
  ...
  public void ukrcajPaketeUVoziloIDostavi() {
    ...
    for (Vozilo vozilo : vozila) {
      if (vozilo.isStatusSlobodno() && (izracunajPostotakPopunjenostiVozila(vozilo.getKapacitet(),
        ...
        for (int i = 0; i < vozilo.listaPaketaZaDostavu.size(); i++) {
          vozilo.listaPaketaZaDostavu.get(i).setStatus("NA DOSTAVI");
        }
